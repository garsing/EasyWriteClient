...
namespace WordAddIn1.PresentationHost
{
    internal static class PowerPointPresentationHost
    {
            ...
            out string error)
        {
            ...
            if (!TryFindSlideBySlideId(presentation, commonSlideId, out PowerPoint.Slide slide, out error))
            {
                return false;
            }
            ...
            for (int i = 0; i < parsed.Count; i++)
            {
                ...
                {
                    error = "读取形状几何失败: " + shapeId + " — " + ex.Message;
                    return false;
                }
            }
            ...
            out string error)
        {
            return TryReadPptHtml(
                channel, slideId, shapeId, fullPage, skipRasterized, false,
                PptConventionHtml.DefaultContentMinArea, out result, out error);
        }
