...
namespace WordAddIn1.PresentationHost
{
    public static class PresentationHostAdapter
    {
        ...
        public static bool TryReadPptHtml(
            ...
            out ToolResult errorResult)
        {
            return TryReadPptHtml(channel, slideId, shapeId, fullPage, false, out result, out errorResult);
        }
            ...
            out ToolResult errorResult)
        {
            ...
            if (channel is PptChannel ppt)
            {
                ok = PowerPointPresentationHost.TryReadPptHtml(
                    ppt, slideId, shapeId, fullPage, skipRasterized, contentOnly, contentMinArea,
                    out result, out error);
            }
            else if (channel is WppChannel wpp)
            {
                ok = WppPresentationHost.TryReadPptHtml(
                    wpp, slideId, shapeId, fullPage, skipRasterized, contentOnly, contentMinArea,
                    out result, out error);
            }
            else
            {
        ...
        internal static bool TryCaptureShape(
            IOperationChannel channel,
            string shapeId,
            ...
            out ToolResult errorResult)
        {
            ...
            if (channel is PptChannel ppt)
            {
                ok = PowerPointPresentationHost.TryCaptureShape(ppt, shapeId, out result, out error);
            }
            else if (channel is WppChannel wpp)
            {
                ok = WppPresentationHost.TryCaptureShape(wpp, shapeId, out result, out error);
            }
            else
            {
                errorResult = new ToolResult
