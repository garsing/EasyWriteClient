...
namespace PptHtmlContentRoundtripTest
{
    ...
    internal static class ContentBatchRunnerWpp
    {
                ...
                for (int i = 0; i < cases.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            SaveQuiet(pres, path);
                            sinceRecycle++;
                            int slideCount = TrySlideCount(pres);
                            Console.WriteLine("  … " + (i + 1) + "/" + cases.Count
                                + " 页=" + (slideCount >= 0
                                    ? slideCount.ToString(CultureInfo.InvariantCulture)
                                    : "?"));
                            if (slideCount < 0)
                            {
                                CloseQuiet(pres);
            ...
            string tag)
        {
            ...
            {
                return ContentGroupHarness.Run(run, presentation, slide, slideId, one, tag, wpp: true, assets: assets);
            }
        ...
        private static int TrySlideCount(object presentation)
        {
            try
            {
